...
// Queries and Mutations for messages
...
// Populate the user for the message
function populateUser(ctx: QueryCtx, userId: Id<"users">) {
  return ctx.db.get(userId);
}

// Populate the member for the message
...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // If the conversationId is not provided, we need to find the conversationId
    if (!args.conversationId && !args.channelId && args.parentMessageId) {
      const parentMessage = await ctx.db.get(args.parentMessageId);
      if (!parentMessage) {
        throw new Error("Invalid parent message");
